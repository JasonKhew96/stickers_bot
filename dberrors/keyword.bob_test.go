...
func TestKeywordUniqueConstraintErrors(t *testing.T) {
	if testDB == nil {
	...
	tests := []struct {
		...
		conflictMods func(context.Context, *testing.T, bob.Executor, *models.Keyword) factory.KeywordModSlice
	}{
		{
			name:        "ErrUniquePkMainKeyword",
			expectedErr: KeywordErrors.ErrUniquePkMainKeyword,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			obj, err := f.NewKeywordWithContext(ctx, factory.KeywordMods.WithParentsCascading()).Create(ctx, exec)
			if err != nil {
				t.Fatal(err)
			}

			obj2, err := f.NewKeywordWithContext(ctx).Create(ctx, exec)
			if err != nil {
				t.Fatal(err)
			}

			err = obj2.Update(ctx, exec, f.NewKeywordWithContext(ctx, tt.conflictMods(ctx, t, exec, obj)...).BuildSetter())
