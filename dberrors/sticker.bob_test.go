...
import (
	...
	factory "github.com/JasonKhew96/stickers_bot/factory"
	models "github.com/JasonKhew96/stickers_bot/models"
	"github.com/stephenafamo/bob"
)

func TestStickerUniqueConstraintErrors(t *testing.T) {
	if testDB == nil {
	...
	tests := []struct {
		...
		conflictMods func(context.Context, *testing.T, bob.Executor, *models.Sticker) factory.StickerModSlice
	}{
		{
			name:        "ErrUniquePkMainSticker",
			expectedErr: StickerErrors.ErrUniquePkMainSticker,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			obj, err := f.NewStickerWithContext(ctx, factory.StickerMods.WithParentsCascading()).Create(ctx, exec)
			if err != nil {
				t.Fatal(err)
			}

			obj2, err := f.NewStickerWithContext(ctx).Create(ctx, exec)
			if err != nil {
				t.Fatal(err)
			}

			err = obj2.Update(ctx, exec, f.NewStickerWithContext(ctx, tt.conflictMods(ctx, t, exec, obj)...).BuildSetter())
